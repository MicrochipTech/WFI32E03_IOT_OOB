...
//---------------------------------------------------------------------------
 ...
 static BOOTLOADER_STATUS Bootloader_Task_JumpApplication(void) {
    ...
    {
            if(( APP_IMG_BOOT_CTL->boot_addr == 0xb0170200) && (APP_IMG_BOOT_CTL->status == IMG_STATUS_DOWNLOADED ) && (APP_IMG_BOOT_CTL->type == IMG_TYPE_PRODUCTION ) && (APP_IMG_BOOT_CTL->order == 0xFF))
            {
                printf("Press 'a' for Cloud Image or 'b' for OTA Downloaded Image\r\n");
                
                char ch;
                ch = getchar();
                if( ch == 'a')
                {
                    fptr = (void(*)(void)) (APP_IMG_BOOT_ADDR);
                }
                else if( ch == 'b')
                {
                    fptr = (void(*)(void)) (APP_IMG_BOOT_CTL->boot_addr);
